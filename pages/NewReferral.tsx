...
const NewReferral: React.FC<NewReferralProps> = ({ 
  ...
  // Form State - Trainee Data (Manual Entry)
  ...
  const isCounselor = currentUser.role === UserRole.COUNSELOR || currentUser.isCounselor; // Check role or flag
  ...
  const handleSubmitNew = () => {
    ...
    const referral: Referral = {
      id: Date.now().toString(),
      traineeId: Date.now().toString(), // Generate generic ID for manual entry
      traineeName: traineeName,
      trainingNumber: trainingNumber,
      department: 'التقنية الميكانيكية',
      // CRITICAL: Save Specialization to route to correct HoD
  ...
  const handleAction = (newStatus: ReferralStatus, actionName: string, requiredSignature: 'hod' | 'counselor' | null) => {
    ...
    if (!currentActionComment && actionName !== 'توجيه') { // Require comment for resolving/escalating
    ...
    const updatedReferral: Referral = {
      ...
      hodSignature: requiredSignature === 'hod' ? true : initialData.hodSignature,
      counselorSignature: requiredSignature === 'counselor' ? true : initialData.counselorSignature,
    };
        ...
        {initialData?.timeline.map((event) => (
          <div key={event.id} className="flex gap-4 bg-gray-50 p-3 rounded-lg border border-gray-200">
            ...
            <div className="flex-1">
              ...
              {event.comment && (
                <p className="text-sm text-gray-700 mt-2 bg-white p-2 rounded border border-gray-100">{event.comment}</p>
              )}
            </div>
          </div>
        ))}
    ...
    <div className="max-w-4xl mx-auto pb-20 md:pb-8">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-6 no-print gap-4">
        <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-start">
          ...
          <button onClick={exportToPDF} className="bg-gray-800 text-white px-4 py-2 rounded-lg hover:bg-gray-900 flex items-center gap-2">
      ...
      <div ref={formRef} className="bg-white p-4 md:p-8 rounded-xl shadow-sm border border-gray-100 print:shadow-none print:border-0">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {canEditTrainerSection ? (
                  ...
                  <p className="text-gray-900 font-medium border-b border-gray-100 pb-1">{traineeName}</p>
                )}
              </div>
              ...
              <div>
                ...
                {canEditTrainerSection ? (
                  <input
                    ...
                    className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 outline-none"
                    placeholder="مثال: 44410523"
                  />
                ) : (
                   <p className="text-gray-900 font-mono border-b border-gray-100 pb-1">{trainingNumber}</p>
                )}
              </div>
              ...
              <div>
                ...
                {canEditTrainerSection ? (
                  ...
                  >
                   ...
                   <p className="text-gray-900 border-b border-gray-100 pb-1">{specialization}</p>
                )}
              </div>
               
              <div>
                 <label className="block text-sm font-bold text-gray-700 mb-1">القسم:</label>
                 <p className="text-gray-900">التقنية الميكانيكية</p>
              </div>
            </div>
              ...
              {Object.values(CaseType).map((type) => (
                <label key={type} className="flex items-center space-x-2 space-x-reverse">
                  <input
                    ...
                    disabled={!canEditTrainerSection}
               ...
               <div className="flex flex-col sm:flex-row gap-4">
                 {Object.values(Repetition).map((rep) => (
                   <label key={rep} className="flex items-center space-x-2 space-x-reverse">
                     <input
                       type="radio"
                       checked={repetition === rep}
                       onChange={() => canEditTrainerSection && setRepetition(rep)}
                       disabled={!canEditTrainerSection}
        ...
        {/* Create Button for New Referral */}
        {isNew && (
        ...
        {!isNew && initialData && (
          <>
            ...
            {/* Actions Section - Only Visible to Actor based on Status */}
            <div className="mt-8 no-print bg-blue-50 p-6 rounded-xl border border-blue-100">
              ...
              <div className="mb-4">
                <div className="flex justify-between mb-2">
                   <label className="block text-sm font-bold text-gray-700">التعليق / الإجراء المتخذ:</label>
                   {isCounselor && (initialData.status === ReferralStatus.PENDING_COUNSELOR) && (
                     ...
                     >
                       <Wand2 size={12} /> {isAnalyzing ? 'جاري التحليل...' : 'استشارة الذكاء الاصطناعي'}
                     </button>
                   )}
                </div>
                <textarea
              ...
              <div className="flex flex-col md:flex-row flex-wrap gap-4">
                ...
                {/* Read Only Message */}
                {initialData.status === ReferralStatus.RESOLVED && (
                   <p className="text-green-700 font-bold flex items-center gap-2"><CheckCircle/> هذه الإحالة مكتملة.</p>
                )}
                 {initialData.status === ReferralStatus.TO_STUDENT_AFFAIRS && (
                   <p className="text-red-700 font-bold flex items-center gap-2"><AlertTriangle/> هذه الإحالة محالة لشؤون المتدربين.</p>
                )}
              </div>
            </div>
          </>
